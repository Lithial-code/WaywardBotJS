...
exports.run = (client, message, args) => {
    ...
    if (member.roles.has(GMRole.id)) {
        ...
        var chan_OOC_TRACKER = message.guild.channels.find(channel => channel.name === `${name}_ooc_tracker`);
        ...
        chan_OOC.delete('Expedition is finished. Deleting channels')
            .then(deleted => console.log(`Deleted ${deleted.name} because its time was done`))
            .catch(console.error);
        chan_OOC_TRACKER.delete('Expedition is finished. Deleting channels')
            .then(deleted => console.log(`Deleted ${deleted.name} because its time was done`))
            .catch(console.error);
